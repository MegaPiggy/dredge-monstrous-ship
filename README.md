...
### Creating your mod
1. [Generate your repository from this template](https://github.com/new?template_name=DredgeMonstrousShip&template_owner=DREDGE-Mods)
2. Clone your new repository to your machine
3. Open all files in a text editor and do a find replace on `DredgeMonstrousShip` to change it to the name of your mod without spaces. For example, `CosmicHorrorFishingBuddies`.
4. Rename all files with `DredgeMonstrousShip` in the name to instead be the name of your mod without spaces.
5. Edit `DredgeMonstrousShip/mod_meta.json`
6. Edit `DredgeMonstrousShip/DredgeMonstrousShip.csproj.user`
7. Open `DredgeMonstrousShip.sln` in Visual studio
8. Start writing your code in `DredgeMonstrousShip.cs`
...
```
{
	...
	"ModGUID": "xen.DredgeMonstrousShip",
	"Version": "0.0.1",
	"ModAssembly": "DredgeMonstrousShip.dll",
	"MinWinchVersion": "0.2.3",
	"Entrypoint": "DredgeMonstrousShip.Loader/Initialize"
}
```
...
- `ModAssembly`: The main `.dll` file for your mod. This should be changed along with all other references to `DredgeMonstrousShip` in a find/replace on all files.
...
- In the `OutputPath` line, change `xen.DredgeMonstrousShip` to match the `ModGUID` you wrote in `mod_meta.json`.
